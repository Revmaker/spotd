...
const urlRegex = new RegExp(
  /(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?/ as RegExp,
  'gi'
);
...
const FEATURE_FULL_NAME = {
  ...
  test: (line: string) => {
    ...
    let tokens = tokenizer
      .tokenize(line)
      .filter((token: string) => !digitsRegex.test(token));
    if (tokens.length < 2) return false;

    let lastToken = tokens[tokens.length - 1];
    return (
      lastToken.toUpperCase() == lastToken && FEATURE_CAPITAL_CASE.test(line)
    );
  },
};
...
const FEATURE_SENTENCE = {
  ...
  test: (line: string) => {
    ...
    let tokensWithoutStopWords = tokens.filter((token: string) => {
      return selectedStopWords.indexOf(token.toLowerCase()) < 0;
    });
...
const FEATURE_CAPITAL_CASE = {
  ...
  test: (line: string) => {
    let words: Array<string> = tokenizer
      .tokenize(line)
      .filter((token: string) => !digitsRegex.test(token));
    let tokensWithoutStopWords = words.filter((token: string) => {
      return selectedStopWords.indexOf(token.toLowerCase()) < 0;
    });
...
const setSelectedStopWords = (
  languageOfCurrentLine: string,
  languageOfWholeEmail: string
) => {
...
export const detectFeaturesInText = function (
  lineOfText: string,
  language: string
): Array<Features> {
  let detectedLangForCurrentLine = detectLanguageOrDefault(
    lineOfText,
    language
  );
  ...
  suppotedFeatures.forEach((feature) => {
    if (feature.test(lineOfText)) detectedFeatures.push(feature.name);
  });

  return detectedFeatures.sort();
};

export function getFeaturesAsVector(
  lineOfText: string,
  language: string
): Array<number> {
  ...
  return featuresArray.map((feature) =>
    detectedFeatures
      .map((detectedFeature) => Features[detectedFeature])
      .indexOf(feature as Features) > -1
      ? 1
      : 0
  );
}
